...
class User extends Controller
{
    public function inputReferal (Request $request) {
        try {
            ...
            if(!$user) {
                return response()->json(['message' => 'referal code yang anda masukan salah', 400]);
            }
            $update = UserModel::find($id);
            if($update['referrer_to'] != '') {
                return response()->json(['message' => 'anda sudah pernah memasukan referal code', 400]);
            }
           
           
            if($update['reference_code'] == $code) {
                return response()->json(['message' => 'anda memasukan referal code sendiri', 400]);
            }

            if($user['role'] == 1) {
                return response()->json(['message' => 'referal code yang anda masukan bukan punya relawan / koordinator', 400]);
            }
            ...
            return response()->json(['message' => 'berhasil memasukan referal code', 200]);
        } catch (\Exception $e) {
